  ...
  Future<T> send<T>(ApiRequest<T> request) async {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (e.error == WINRError.authenticationFailed && _refreshHandler != null) {
          Logger.instance.debug('Authentication failed, attempting token refresh');
        ...
        if (attempt >= maxRetries - 1) {
          throw WINRException(WINRError.networkError);
        }
        attempt++;
    ...
    try {
      ...
      switch (request.method) {
        ...
        case 'POST':
          ...
          response = await client.post(url, headers: headers, body: body).timeout(timeout);
          break;
        case 'PUT':
          final body = request.body != null ? jsonEncode(request.body) : null;
          response = await client.put(url, headers: headers, body: body).timeout(timeout);
          break;
        case 'DELETE':
          response = await client.delete(url, headers: headers).timeout(timeout);
          break;
        default:
          throw WINRException(WINRError.unknown);
      }
      
      Logger.instance.debug('Response: ${response.statusCode} ${response.reasonPhrase}');
    ...
    switch (statusCode) {
      case 400:
        if (errorMessage?.contains('underage') == true) {
          throw WINRException(WINRError.underage);
        }
        if (errorMessage?.contains('invalid email') == true) {
          throw WINRException(WINRError.invalidEmail);
        }
        throw WINRException(WINRError.invalidApiKey);
      case 401:
        throw WINRException(WINRError.authenticationFailed);
      case 403:
        if (errorMessage?.contains('geography') == true) {
          throw WINRException(WINRError.geographyNotAllowed);
        }
        if (errorMessage?.contains('already claimed') == true) {
          throw WINRException(WINRError.ineligibleToday);
        }
        throw WINRException(WINRError.authenticationFailed);
      case 404:
        throw WINRException(WINRError.campaignNotAvailable);
      case 429:
        throw WINRException(WINRError.networkError); // Rate limited
      case 500:
      case 502:
      case 503:
      case 504:
        throw WINRException(WINRError.serverError);
      default:

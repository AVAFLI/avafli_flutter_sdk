...
/// Manages push notification token registration with the Avafli backend.
///
...
/// });
/// ```
class AvafliPushNotificationManager {
  static final AvafliPushNotificationManager _instance =
      AvafliPushNotificationManager._internal();
  static AvafliPushNotificationManager get instance => _instance;
  ...
  ///
  /// Safe to call before Avafli.configure() — the token will be queued
  ...
  Future<void> didReceiveRegistrationToken(String token) async {
    ...
    if (_networkClient == null) {
      _pendingToken = token;
      Logger.instance
          .debug('Avafli SDK not configured yet, queuing push token');
      return;
    }
  ...
  /// Registers the push token with the Avafli backend.

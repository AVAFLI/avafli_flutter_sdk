...
abstract class StreakEngineProtocol {
  ...
  Result<StreakState, WINRError> nextState(StreakState? currentState, DateTime claimDate);
...
class StreakEngine implements StreakEngineProtocol {
  ...
  Result<StreakState, WINRError> nextState(StreakState? currentState, DateTime claimDate) {
    ...
    final newDailyStreak = _calculateDailyStreak(currentState.currentDay, daysDifference);
  ...
  Result<StreakState, WINRError> _createInitialState(DateTime utcDate, [int totalEntries = 0]) {
  ...
  (int, String) _calculateWeeklyProgress(StreakState currentState, String currentWeekStart) {
  ...
  (int, String) _calculateMonthlyProgress(StreakState currentState, String currentMonthFirst) {
...
class Result<T, E> {
  ...
  Result<U, E> map<U>(U Function(T) fn) {
    return _isSuccess ? Result.success(fn(_value!)) : Result.error(_error!);
  }
  
  /// Maps the error to a new type.
  Result<T, F> mapError<F>(F Function(E) fn) {
    return _isSuccess ? Result.success(_value!) : Result.error(fn(_error!));
  }
}